...
export default {
    ...
    name: 'purchases-orders-export-pdf',
    props: ['purchase_order_id', 'headers'],
    data() {
        return {
            purchase_order: {},
    ...
    methods: {
        ...
        async get_details_order() {
            try {
                ...
                let response = await window.axios.get(`get_purchase_order_details/${this.purchase_order_id}`);

                if (response.status != 200)
                    throw new Error(response.data.message);

                this.purchase_order = response.data.purchase_order;
                this.purchase_order_items = response.data.purchase_order_items;
        ...
        async exportPdf() {
            ...
            autoTable(doc, {
                body: [
                    [
                        {
                            content: 'Ingreso de Materia Prima',
                            styles: {
                                halign: 'center',
                                fontSize: 16,
            ...
            autoTable(doc, {
                head: headers,
                body: this.purchase_order_items,
                theme: 'striped',
                headStyles: {
                    fillColor: '#343a40'
                }
            ...
            autoTable(doc, {
                body: [
                    ...
                    [
                        {
                            content: 'Nombre: ' + this.provider.name,
                            styles: {
                                halign: 'left'
                            }
                        },

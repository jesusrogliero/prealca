<template>
	<div>
		...
		<!-- Tabla de datos -->
		<data-table ref="gridbox" url="primaries_products" :headers="headers" title="Productos Primarios" :add="openDialog"
		...
		<dialog-confirm :active="dialog == 'delete'" :confirm="destroyOrder" :cancel="cleanForm">
			<template v-slot:dialog-title>Eliminar Materia Prima?</template>
					...
					<v-row>
						...
						<v-col cols="6" class="mt-2">
							<v-text-field v-model="name" :rules="nameRules" label="Nombre"
								required placeholder="Ingresa el nombre del Producto"></v-text-field>
						</v-col>

						<v-col cols="6" class="mt-2">
							<v-text-field v-model="stock" :rules="stockRules" suffix="Kg" label="Existencia"
								required placeholder="Ingresa la existencia del producto"></v-text-field>
						</v-col>
	...
	methods: {
		...
		async create() {
			try {
				...
				let response = await window.axios.post('primaries_products', {
					name: this.name,
		...
		async update() {
			try {
				let response = await window.axios.put(`primaries_products/${this.id}`, {
					name: this.name,
					stock: this.stock,
				});
		...
		async destroyOrder() {
			try {
				let response = await window.axios.delete(`primaries_products/${this.id}`);

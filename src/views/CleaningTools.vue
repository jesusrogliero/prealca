<template>
	...
	<v-container>
		...
		<v-data-table :headers="headers" :items="cleaning_tools" sort-by="calories" class="elevation-0 ms-n5 mt-n9"
			hide-default-footer @page-count="pageCount = $event" :page.sync="page" :items-per-page="16"
			:search="search">
			<template v-slot:top>
				<v-toolbar flat>
					<v-toolbar-title>Utencilios De Limpieza Y Desinfección</v-toolbar-title>
...
export default {
	...
	computed: {
		formTitle() {
			return this.editedIndex === -1 ? 'Nuevo Utensilio' : 'Actualizar un Utensilio';
		}
	},
	...
	methods: {
		...
		initialize: async function () {
			try {
				let response = await window.axios.get('/cleaning_tools');

				if (response.status == 200)
					this.cleaning_tools = response.data;
				else
					throw new Error(response.data);

				if (Math.round(Object.keys(this.cleaning_tools).length / 16) >= 1)
					this.pageCount = Math.round(Object.keys(this.cleaning_tools).length / 16);
		...
		editItem: async function (item) {
			try {
				...
				let response = await window.axios.get(`/cleaning_tools/${item.id}`);
		...
		deleteItemConfirm: async function () {
			try {
				...
				let response = await window.axios.delete(`/cleaning_tools/${this.editedIndex}`);
		...
		save: async function () {
			try {
				...
				if (this.editedIndex > -1)
					response = await window.axios.put(`/cleaning_tools/${this.editedItem.id}`, this.editedItem);
				else
					response = await window.axios.post('/cleaning_tools', this.editedItem);

<template>
	...
	<v-container>
		...
		<v-data-table :headers="headers" :items="users" sort-by="calories" class="elevation-0 ms-n5 mt-n9"
			...
			<template v-slot:top>
				<v-toolbar flat>
									...
									<v-row>
										...
										<v-col cols="12" v-show="editedIndex === -1">
											<v-text-field v-model="editedItem.password" type="password"
												label="Ingresa una contraseña"></v-text-field>
										</v-col>

										<v-col cols="12" v-show="editedIndex === -1">
											<v-text-field v-model="editedItem.password_confirm" type="password"
												label="Vuelve a ingresar la contraseña"></v-text-field>
										</v-col>

										<v-col cols="12">
											<autocomplete-form uri="/roles" label="Selecciona el Cargo" column="name"
												itemValue="id" :defaultValue="editedItem.role_id"
												:getSelect="getSelectRole" :key="key_component" />
										</v-col>
					...
					<v-dialog v-model="dialogDelete" max-width="600px">
						<v-card>
							<v-card-title class="text-h5">Estas seguro que deseas eliminar este usuario?</v-card-title>
							<v-card-actions>
								<v-spacer></v-spacer>
...
export default {
	...
	computed: {
		formTitle() {
			return this.editedIndex === -1 ? 'Registrar Nuevo Usuario' : 'Actualizar Informacion de Usuario';
		}
	},
	...
	methods: {
		...
		initialize: async function () {
			try {
				let response = await window.axios.get('/users');
				...
				if (Math.round(Object.keys(this.users).length / 16) >= 1)
					this.pageCount = Math.round(Object.keys(this.users).length / 16);
		...
		editItem: async function (item) {
			try {
				...
				let response = await window.axios.get(`/users/${item.id}`);
		...
		deleteItemConfirm: async function () {
			try {
				...
				let response = await window.axios.delete(`/users/${this.editedIndex}`);
		...
		save: async function () {
			try {
				...
				if (this.editedIndex > -1)
					response = await window.axios.put(`/users/${this.editedItem.id}`, this.editedItem);
				else
					response = await window.axios.post('/users', this.editedItem);

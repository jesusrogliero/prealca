<template>
    <div>
        <v-btn color="error" icon class="mb-2" @click="exportPdf" >
    ...
    methods: {
        ...
        async get_details_order() {
            try {
                ...
                let response = await window.axios.get(`dispatches/${this.dispatch_id}`);
                ...
                response = await window.axios.get(`dispatch_items/${this.dispatch_id}`);

                if (response.status != 200)
                    throw new Error(response.data.message);

                this.dispatch_item = response.data;

                response = await window.axios.get(`receivers/${this.dispatch.receiver_id}`);
        ...
        async exportPdf() {
            ...
            autoTable(doc, {
                body: [
                    ...
                    [
                        {
                            content: 'Nombre: ' + this.receiver.name,
                            styles: {
                                halign: 'left'
                            }
                        },
                        ...
                        {
                            content: this.formatNumber(this.purchase_order.total_products),
                            styles: {
                                halign: 'right'
                            }
                        },
                    ...
                    [
                        {
                            content: 'C.I o RIF: ' + this.provider.identityF,
                            styles: {
                                halign: 'left'
                            }
                        },
                        ...
                        {
                            content: this.formatNumber( this.purchase_order.total_load ) + ' Kg',
                            styles: {
                                halign: 'right'
                            }
                        },
                    ...
                    [
                        {
                            content: 'Telefono: ' + this.provider.phone,
                            styles: {
                                halign: 'left'
                            }
                        },
                        ...
                        {
                            content: 'Total No Conforme:',
                            styles: {
                                halign: 'right'
                            }
                        },
                        {
                            content: this.formatNumber( this.purchase_order.total_nonconforming ) + ' Kg',
                            styles: {
                                halign: 'right'
                            }
                        },
                    ...
                    [
                        {
                            content: 'Dirección: ' + this.provider.address,
                            styles: {
                                halign: 'left'
                            }
                        },
                        ...
                        {
                            content: 'Nº de Guia SADA: ',
                            styles: {
                                halign: 'right'
                            }
                        },
                        {
                            content: this.purchase_order.nro_sada_guide,
                            styles: {
                                halign: 'right'
                            }
                        },
            ...
            autoTable(doc, {
                body: [
                    ...
                    [
                        {
                            content:  this.purchase_order.observations == null ? "Sin Observaciones" : this.purchase_order.observations,
                            styles: {
                                halign: 'left'
                            }
                        }
            ...
            return doc.save("reporte_orden_de_ingreso");
        },

<template>
	...
	<v-container>
		...
		<v-data-table :headers="headers" :items="maquinaries_consumible" sort-by="calories"
			class="elevation-0 ms-n5 mt-n9" hide-default-footer @page-count="pageCount = $event" :page.sync="page"
			:items-per-page="16" :search="search" v-model="prueba">
			<template v-slot:top>
				<v-toolbar flat>
					<v-toolbar-title>Servicios y Consumibles de Maquinarias y Equipos</v-toolbar-title>
					...
					<v-dialog v-model="dialogDelete" max-width="600px">
						<v-card>
							<v-card-title class="text-h5">Estas seguro que deseas eliminar este Consumible?
							</v-card-title>
							<v-card-actions>
								<v-spacer></v-spacer>
...
export default {
	...
	computed: {
		formTitle() {
			return this.editedIndex === -1 ? 'Nuevo Consumible' : 'Actualizar un Consumible';
		}
	},
	...
	methods: {
		...
		initialize: async function () {
			try {
				let response = await window.axios.get('/machineries_consumables');

				if (response.status == 200)
					this.maquinaries_consumible = response.data;
				else
					throw new Error(response.data);

				if (Math.round(Object.keys(this.maquinaries_consumible).length / 16) >= 1)
					this.pageCount = Math.round(Object.keys(this.maquinaries_consumible).length / 16);
		...
		editItem: async function (item) {
			try {
				...
				let response = await window.axios.get(`/machineries_consumables/${item.id}`);
		...
		deleteItemConfirm: async function () {
			try {
				...
				let response = await window.axios.delete(`/machineries_consumables/${this.editedIndex}`);
		...
		save: async function () {
			try {
				...
				if (this.editedIndex > -1)
					response = await window.axios.put(`/machineries_consumables/${this.editedItem.id}`, this.editedItem);
				else
					response = await window.axios.post('/machineries_consumables', this.editedItem);

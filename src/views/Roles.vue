<template>
	...
	<v-container>
		...
		<v-data-table :headers="headers" :items="roles" sort-by="calories" class="elevation-0 ms-n5 mt-n9"
			hide-default-footer @page-count="pageCount = $event" :page.sync="page" :items-per-page="16"
			:search="search">
			<template v-slot:top>
				<v-toolbar flat>
					<v-toolbar-title>Administración de los Cargos en la Plata</v-toolbar-title>
						...
						<v-card>
							...
							<v-card-text>
								...
								<v-container>
									<v-row>
										<v-col cols="12">
											<v-text-field v-model="editedItem.name" label="Nombre del Cargo">
											</v-text-field>
										</v-col>
					...
					<v-dialog v-model="dialogDelete" max-width="600px">
						<v-card>
							<v-card-title class="text-h5">Estas seguro que deseas eliminar este cargo?</v-card-title>
							<v-card-actions>
								<v-spacer></v-spacer>
...
export default {
	...
	computed: {
		formTitle() {
			return this.editedIndex === -1 ? 'Registrar Nuevo Cargo' : 'Actualizar un Cargo';
		}
	},
	...
	methods: {
		...
		initialize: async function () {
			try {
				let response = await window.axios.get('/roles');
				...
				if (Math.round(Object.keys(this.roles).length / 16) >= 1)
					this.pageCount = Math.round(Object.keys(this.roles).length / 16);
		...
		editItem: async function (item) {
			try {
				...
				let response = await window.axios.get(`/roles/${item.id}`);
		...
		deleteItemConfirm: async function () {
			try {
				...
				let response = await window.axios.delete(`/roles/${this.editedIndex}`);
		...
		save: async function () {
			try {
				...
				if (this.editedIndex > -1)
					response = await window.axios.put(`/roles/${this.editedItem.id}`, this.editedItem);
				else
					response = await window.axios.post('/roles', this.editedItem);
